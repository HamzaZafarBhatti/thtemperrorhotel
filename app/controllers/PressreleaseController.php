	...
	*/
	...
	/* Admin Functions */
        public function adminPressrelease()
        {
		  ...
		  $pressreleases = Pressrelease::orderBy('updated_at','DESC')->paginate($noperpage);
         ...
         public function frontPressrelease($dateSort = null)
        {
	          $page = Page::where('type','=','pressrealese')->where('published','=',1)->get();
                   if(isset($dateSort) != '')
                  {
                      ...
                      $pressreleases = Pressrelease::select('id','title','citation','content','read_more',DB::raw('SUBSTRING(date, -4) as year'),'created_at','updated_at','image_file_name','image_file_size','image_content_type','image_updated_at','pdf_file_name','pdf_file_size','pdf_content_type','pdf_updated_at')->where('active','=',1)->orderBy(DB::raw('SUBSTRING(date, -4)'),'DESC')->get();

                  }
                  $categories = Pressrelease::select('id',DB::raw('SUBSTRING(date, -4) as year'),DB::raw('count(id) as count'))->where('active','=',1)->groupBy(DB::raw('SUBSTRING(date, -4)'))->orderBy(DB::raw('SUBSTRING(date, -4)'),'DESC')->get();
