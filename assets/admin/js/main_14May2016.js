$(function () {
 ...
 $('.deleteidnews').click(function () {
       var page = $(this).attr('rel');
       var idloc = $(this).attr('rev');
       if ($('.chkNews:checked').length) {
          var chkId = '';
...
$('.updateOrder').click(function() {
   ...
   $('.display_order1').each(function () {
     ...
     $('.deletePDF').click(function(event) {
         ...
         var Model = $('#deletePDFMname').val();
      
          $.ajax({
                   type: "POST",
                   url: "/~yeelee72185perak/laravel/public/admin/deletepdf",
                  // The key needs to match your method's input parameter (case-sensitive).
                  data: { RecId: recId , model:Model},
                  success: function(data){
                                      $('#pdfCont').hide();
                                      },
                  failure: function(errMsg) {
