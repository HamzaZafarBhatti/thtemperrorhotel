...
//echo '<pre>'; print_r($slidingbanners); die;
					 if($pgCount > 0)
           {
           $cntarray1 = $cntArr;
           }
...
//echo '<pre>'; print_r(Input::all()); die;
            ...
            $files2 = Input::file('bannerimage');

            $mainImage = $files2->getClientOriginalName();
//echo '<pre>'; print_r($mainImage); 

//echo $files2->getSize();
//echo $files2->getClientMimeType();
//die;
            $ext = explode('.', $mainImage);
            $ext = strtolower($ext[count($ext) - 1]);
            $upload_success = $files2->move($destinationPath, $mainImage);


$slidingbanner = new Slidingbanner();
   $slidingbanner->title = $data['title'];
   $slidingbanner->display_order = $data['display_order'];
   $slidingbanner->active= $data['active']; 
$slidingbanner->type= $data['type']; 
   $slidingbanner->bannerimage_file_name = $mainImage;
$slidingbanner->bannerimage_file_size = $files2->getSize();
 $slidingbanner->bannerimage_content_type = $files2->getClientMimeType();
$slidingbanner->bannerimage_updated_at=date('Y-m-d H:i:s');
$slidingbanner->save();
