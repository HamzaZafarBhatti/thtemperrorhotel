...
class PageController extends BaseController {
    ...
    public function adminDeleteAllBanner() {
         ...
         if($data['type'] == 'technology') {
             $slidingbanners = Slidingbanner::where('type','=','technology')->delete();
} else {
          $slidingbanners = Slidingbanner::where('type','=','company-profile')->delete();
}
    ...
    public function adminDeleteMultipleEnquirycategory() {
        $id = Input::get('deleteid');
        $ids = explode(',', $id[0]);
        $banner = Enquirycategory::whereIn('id', $ids)->delete();
        //  $banner->delete();
        if ($banner) {
            return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
     ...
     public function adminDeleteMultipleEnquirysubcategory() {
        $id = Input::get('deletesubid');
        $ids = explode(',', $id[0]);
        $banner = Enquirycategory::whereIn('id', $ids)->delete();
        //  $banner->delete();
        if ($banner) {
    ...
    public function adminDeleteMultipleBoard() {
        $id = Input::get('deleteid');
        $ids = explode(',', $id[0]);
        $banner = Boardcharter::whereIn('id', $ids)->delete();
        if ($banner) {
            return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
                <i class="fa fa-check-circle"></i> <strong>Success!</strong>
    ...
    public function adminDeleteMultipleUserlist() {
     ...
     public function adminDeleteMultipleEnquiryemail() {
        $id = Input::get('deleteid');
        $ids = explode(',', $id[0]);
        $banner = Enquiryemail::whereIn('id', $ids)->delete();
        //  $banner->delete();
        if ($banner) {
            return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
